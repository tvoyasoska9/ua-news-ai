...
class ModerationBot:
    ...
    def keyboard(self,item_id):
        return InlineKeyboardMarkup([[InlineKeyboardButton("PUBLISH",callback_data=f"publish:{item_id}"),InlineKeyboardButton("REJECT",callback_data=f"reject:{item_id}")]])
    ...
    async def send_for_moderation(self,item,raw):
        ...
        text=self.render(item.title,item.text)+"\n\nORIGINAL POST:\n"+raw.url
        await self.app.bot.send_message(self.settings.moderation_chat_id,text,parse_mode="HTML",reply_markup=self.keyboard(item_id),disable_web_page_preview=True)
    ...
    async def callback(self,update,context):
        ...
        if not data: await q.answer("Already processed"); return
        await q.answer()
        if action=="reject":
            self.db.set_status(data["url"],"rejected"); self.db.delete_pending(item_id); await q.edit_message_reply_markup(reply_markup=None); return
        try:
            media=data["media"] or []
            for start in range(0,len(media),10):
                chunk=media[start:start+10]
                await self.app.bot.send_media_group(self.settings.publish_channel_id,[InputMediaVideo(fid) if kind=="video" else InputMediaPhoto(fid) for fid,kind in chunk])
            ...
            self.db.set_status(data["url"],"published"); self.db.delete_pending(item_id); await q.edit_message_reply_markup(reply_markup=None)
