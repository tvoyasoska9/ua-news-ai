...
TELEGRAM_POST_LIMIT = 8
TELEGRAM_MAX_AGE_HOURS = 3
...
async def fetch_telegram_source(client, source):
    ...
    for message in messages:
        ...
        title = " ".join(text.split())[:180] if text else "Новинний медіапост"
        published_at = (
        ...
        if media_type:
            try:
                ...
                log.exception(
                    ...
                    media_type, username, message_id
                )
                media_type = None
                media_path = None

        # Media-only posts have no factual material for the current AI editor.
        if not text:
            if media_path:
                try:
        ...
        # Equal hard cap for every configured channel on each poll.
