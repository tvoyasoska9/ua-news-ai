...
async def fetch_telegram_source(client, source):
    ...
    log.info(
        "Telegram source @%s checked: %s text posts collected",
        username,
        len(items),
    )
    return items
...
async def collect_telegram_news(settings):
    ...
    log.info("Telegram monitor checking %s channels", len(TELEGRAM_SOURCES))
    ...
    log.info("Telegram monitor collected %s candidates", len(items))
    return items
...
async def collect_news(settings):
    ...
    return list(rss_items) + list(telegram_items)
