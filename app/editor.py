...
ЗБЕРЕГТИ ПРИБЛИЗНО ТУ САМУ КІЛЬКІСТЬ ІНФОРМАЦІЇ Й ОБСЯГ, ЩО Є В ОРИГІНАЛІ.
...
class NewsEditor:
    ...
    async def _rewrite_body(self, material, title, current_text, source, reason):
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Виправ текст української новини. Поверни тільки JSON: "
                        "{\\\"text\\\":\\\"...\\\"}. "
                        "Не додавай жодних нових фактів і не згадуй джерела. "
                        "Збережи приблизно той самий обсяг, абзаци та HTML-виділення "
                        "оригіналу. Основний текст НЕ МОЖЕ повторювати заголовок іншими словами: "
                        "він має містити тільки додаткові деталі. Якщо додаткових деталей немає, "
                        "поверни порожній text."
                    ),
                },
                {
                    "role": "user",
                    "content": (
                        f"ПРИЧИНА ВИПРАВЛЕННЯ: {reason}\n\n"
                        f"ЗАГОЛОВОК:\n{title}\n\n"
                        f"ОРИГІНАЛ:\n{material}\n\n"
                        f"ПОТОЧНИЙ TEXT:\n{current_text}"
                    ),
        ...
        return sanitize_news_html(data.get("text") or "", source)
    ...
    async def edit(self, news):
        ...
        plain_original = _plain(material)
        plain_result = _plain(text)
        too_long = (
            len(plain_original) >= 80
            and len(plain_result) > int(len(plain_original) * 1.25) + 50
        )
        repeated_title = _title_repeated_in_body(title, text)

        if too_long or repeated_title:
            reasons = []
            if too_long:
                reasons.append("text довший за оригінал і штучно роздутий")
            if repeated_title:
                reasons.append("перший абзац повторює зміст заголовка")
            candidate = await self._rewrite_body(
                material, title, text, news.source, "; ".join(reasons)
            )
            # An empty body is valid when the original contains no details beyond
            # the headline. Otherwise keep the corrected version.
            if candidate or repeated_title:
                text = candidate
