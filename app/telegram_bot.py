...
class NewsBot:
    ...
    async def testnews(self, update, context):
        if self.allowed(update):
            ...
            await self.send_for_moderation(item, "test://" + str(uuid4()), None)
    ...
    async def send_for_moderation(self, item, url, image_url=None):
        item_id = str(uuid4())
        self.pending[item_id] = (item, url, image_url)
        keyboard = self.keyboard(item_id)
        text = self.moderation_text(item)
        ...
        if image_url:
            try:
                ...
                # Нерабочая ссылка на изображение не должна ломать новость.
            ...
            if image_url:
                try:
                    await self.app.bot.send_photo(
                        ...
                        photo=image_url,
                        caption=self.publish_text(item),
                        parse_mode="HTML",
                    )
